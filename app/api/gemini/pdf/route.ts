...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (file.size > 4 * 1024 * 1024) return NextResponse.json({ error: "For this Vercel build, keep the PDF under 4 MB. Larger training packages will use a direct cloud-file upload in the next storage/auth phase." }, { status: 413 });
    ...
    let parsed: unknown;
    try { parsed = JSON.parse(raw); } catch { parsed = { summary: raw, courses: [], keyTopics: [], sampleQuestions: [] }; }
    return NextResponse.json(parsed);
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Gemini could not process the PDF." }, { status: 500 });
  }
}
